...
```{r}
DeltaMonthCounts <- count(DeltasClean, Delta, month)
...
```{r}
#find deltas missing a given month
DeltaMonthCounts <- count(DeltaMonthCounts, Delta)
EnoughMonths <- DeltaMonthCounts %>%
 filter( n == 12)
A <- pull(EnoughMonths, Delta)
#remove them
DeltasClean <- DeltasClean %>%
  filter(Delta %in% A)
#add the real dates in month date format
DeltasClean$date <- as.Date(paste(DeltasClean$year, DeltasClean$month, "01", sep="-"), "%Y-%m-%d")
```
...
DeltaMaxMinDisplay
ggplot(DeltaMaxMinDisplay, aes(y = Delta, x = MaxOffset)) + geom_point() + 
  scale_x_discrete(limits = c(1:6), breaks = c(1:6)) +
  expand_limits(x = c(0,6))  + 
  ggtitle("MaxOffset")
ggplot(DeltaMaxMinDisplay, aes(y = Delta, x = MinOffset)) + geom_point() + 
  scale_x_discrete(limits = c(1:6), breaks = c(1:6)) +
  expand_limits(x = c(0,6))  + 
  ggtitle("MinOffset")
ggplot(DeltaMaxMinDisplay, aes(y = Delta, x = OffsetDiff)) + geom_point() + 
  scale_x_discrete(limits = c(1:6), breaks = c(1:6)) +
  expand_limits(x = c(0,6))  + 
...
```{r}
slice1 <- ggplot(DeltaMaxMinDisplay, aes(y = rangeNDVI, x = MaxOffset)) + geom_point() 
# + scale_x_discrete(limits = c(1:6), breaks = c(1:6)) + expand_limits(x = c(1,6)) 
slice2 <- ggplot(DeltaMaxMinDisplay, aes(y = rangeNDVI, x = rangeNDSSI)) + geom_point() 
slice3 <- ggplot(DeltaMaxMinDisplay, aes(y = rangeNDSSI, x = MaxOffset)) + geom_point() 
...
```{r}
DeltaDispLocation <- left_join(DeltaMaxMinDisplay, DeltaLocations, by = c("Delta" = "Deltas"))
```

plot params vs lat
```{r}
loc1 <- ggplot(DeltaDispLocation, aes(y = abs(Lat), x = MaxOffset)) + geom_point() 
loc2 <- ggplot(DeltaDispLocation, aes(y = abs(Lat), x = rangeNDSSI)) + geom_point() 
loc3 <- ggplot(DeltaDispLocation, aes(y = abs(Lat), x = rangeNDVI)) + geom_point() 
loc4 <- ggplot(DeltaDispLocation, aes(y = abs(Lat), x = MaxMeanNDVI)) + geom_point() 
loc5 <- ggplot(DeltaDispLocation, aes(y = abs(Lat), x = MaxMeanNDSSI)) + geom_point() 
...
DeltaPlotter <- function(DeltaName) {
  ...
  #LAND
  Veg <-
    ggplot(data = filter(DeltasClean, Delta == DeltaName &
                           surface == "Land")) +
    ...
    geom_boxplot(
      data = filter(
        DeltasClean,
        Delta == DeltaName &
          surface == "Land" & month == DeltaMaxNDVI$MaxMeanNDVImonth[DeltaMaxNDVI$Delta ==DeltaName] 
      ),
