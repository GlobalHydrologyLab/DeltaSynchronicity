...
def clipImage(image):
  return image.clip(feature.geometry())
...
def masking(image):
  ...
  road = ee.FeatureCollection("TIGER/2016/Roads").filterBounds(feature.geometry()).geometry().buffer(30) 
  ...
  pWater = water.clip(feature.geometry())
...
def featurePull(image):
  f = AddFmask(image).select('fmask')
  clouds = f.eq(2).Or(f.eq(3)).Or(f.eq(4))
  cScore = clouds.reduceRegion(ee.Reducer.mean(),feature.geometry(), 30).get('fmask')
  d = Dswe(image).select('dswe')
  h = CalcHillShades(image, feature.geometry()).select('hillShade')
  
  if mask == "Pekel":
    pixOut = masking(image).addBands(f).addBands(d).addBands(h)
  elif mask == 'dswe':
    pixOut = image.addBands(f).addBands(d).addBands(h)\
    .updateMask(d.eq(1)).updateMask(clouds.eq(0)).set({'cScore':cScore})
      #.updateMask(f.lt(2))
  elif mask == 'none':
    pixOut = image.addBands(f).addBands(d).addBands(h).set({'cScore':cScore})
  elif mask == 'land':
    pixOut = image.addBands(f).addBands(d).addBands(h)\
  ...
  lsout = pixOut.reduceRegion(ee.Reducer.median(), feature.geometry(), 30)
  
  #Collect reflectance and occurence st.dev.
  lsdev = pixOut.reduceRegion(ee.Reducer.stdDev(), feature.geometry(), 30)
  ...
  .set({r.idCol:feature.get(r.idCol)})\
  .set({'sat': mission})\
  ...
  .set({"pixelCount": pixOut.reduceRegion(ee.Reducer.count(), feature.geometry(), 30).get('Blue')})\
...
def featureVar(image):
  
  if PekelMask == True:
    pixOut = masking(image)
  ...
  if PekelMask == False:
    def waterOnly(image):
      ...
      cScore = cScore.reduceRegion(ee.Reducer.mean(),feature.geometry(), 30).get('fmask')
      d = Dswe(image).select('dswe')
      h = CalcHillShades(image, feature.geometry()).select('hillShade')
      return image.addBands(date).addBands(f).addBands(d).addBands(h)\
      .updateMask(d.eq(1).Or(d.eq(2))).set({'cScore':cScore})
      ##.updateMask(f.lt(2))
    
    pixOut = waterOnly(image)

  #Collect median reflectance and occurance values
  lsVar = (pixOut.sample(region = feature.geometry(), 
  scale = 30, projection = pixOut.select('Blue').projection(), 
...
## Big one for actually doing the pull
